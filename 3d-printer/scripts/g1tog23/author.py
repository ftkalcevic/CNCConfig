...
def rad1(x1,y1,x2,y2,x3,y3):
    ...
    #print >>sys.stderr,  "rad1", x1, y1, x2, y2, x3, y3
...
def arc_dir(plane, c, p1, p2, p3):
    ...
    #print "arc_dir", theta_start, theta_mid, theta_end

    if theta_start < theta_end:
        if theta_start < theta_mid:
            ret = True
        else:
            ret = False
    else:
    ...
    #print "arc_dir ", "CCW" if ret else "CW"
    return ret
...
"""
    ...
    for i, p in enumerate(st): 
        ...
        if i > 0:
            path_length = path_length + math.hypot(p[0]-st[i-1][0], p[1]-st[i-1][1])
	...
	c1, c2 = arc_center(plane, ps, st[max_arc], pe)
        #print "arc center", c1, c2
	lx, ly, lz, la, lf = st[0]
	if one_quadrant(plane, (c1, c2), ps, st[max_arc], pe):
	    for i, (x,y,z,a,f) in enumerate(st):
	...
	#print "douglas", len(st), "\n\t", st[0], "\n\t", st[max_arc], "\n\t", st[-1]
	#print "\t", worst_arc_dist, worst_dist
	#print "\t", c1, c2
        #print "chord_length", chord_length(plane,(c1,c2),ps,st[max_arc], pe) 
        #print "path_length", path_length
    if worst_arc_dist < tolerance and worst_arc_dist < worst_dist and abs(chord_length(plane,(c1,c2),ps,st[max_arc], pe) - path_length) < (len(st)-1)*tolerance :
