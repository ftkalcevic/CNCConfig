...
class emc_status:
    ...
    def periodic(self):
        ...
        diaMode = not (80 in self.emcstat.gcodes)
        ...
        if (am & 1):
            ...
            if lathe:
                if diaMode:
                    ...
                    set_text(self.absolute[d], h + fmt % ('D', p[0] * 2.0) )
                    ...
                    set_text(self.distance[d], '(' + fmt % ('D', dtg[0] * 2.0) + ')')
                 #if diaMode:
                    #set_text(self.relative[d], fmt % ('D', relp[0] * 2.0) + ' (' + fmt % ('R', relp[0])+')')
                    #set_text(self.absolute[d], " " + fmt % ('D', p[0] * 2.0) + ' (' + fmt % ('R', p[0])+')')
                    #set_text(self.distance[d], fmt % ('D', dtg[0] * 2.0) + ' (' + fmt % ('R', dtg[0])+')')
                #else:
                    #set_text(self.relative[d], fmt % ('R', relp[0]) + ' (' + fmt % ('D', relp[0] * 2.0) + ')')
                    #set_text(self.absolute[d], h + fmt % ('R', p[0]) + ' (' + fmt % ('D', p[0] * 2.0) + ')')
                    #set_text(self.distance[d], fmt % ('R', dtg[0]) + ' (' + fmt % ('D', dtg[0] * 2.0) + ')')
            else:
